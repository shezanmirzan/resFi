...
from utils import ath9kSpectralScanHelper
from utils import ath9kSpectralScanHelper
...
class AbstractConnector(ResFiSouthBoundAPI):
    def __init__(self, log):
        ...
        self.ath9k_Scan_helper = ath9kSpectralScanHelper(log)
        self.freq = 0
        self.channel = 0
...
class LinuxConnector(AbstractConnector):
    ...
    def get_psd_vector_ht20(self,hdr, buf, debug=False):
        ...
        # check input
        if (len(buf) != (self.ath9k_Scan_helper.SPECTRAL_HT20_NUM_BINS)):
            ...
            noise = int(hdr['noise'])
            rssi = int(hdr['rssi'])
            psd_vector = np.full((self.ath9k_Scan_helper.SPECTRAL_HT20_NUM_BINS), (np.nan), dtype=np.float64)
    ...
    def scan(self,iface='wlan0', q=Queue.Queue(), debug=False):
        ...
        debugfs_dir = self.ath9k_Scan_helper.get_debugfs_dir(iface)
        ...
        if buf:
            ...
            while seek < len(buf):
                ...
                tlv = np.frombuffer(tlv_buf, dtype=self.ath9k_Scan_helper.DTYPE_PSD_TLV)
                tlv_len = self.ath9k_Scan_helper.DTYPE_PSD_TLV.itemsize

                # read sample header
                dtype_psd_hdr = self.ath9k_Scan_helper.SAMPLE_HDR_DTYPE[int(tlv['type'])]
                ...
                # read and decode PSD
                psd_num_bins = self.ath9k_Scan_helper.SAMPLE_NUM_BINS[int(hdr['type'])]
