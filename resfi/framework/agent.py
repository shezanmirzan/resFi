...
class ResFiAgent(ResFiNorthBoundAPI):
    ...
    """
    Dealing with Spectrum sensing Vectors

    """
    def get_psd_vector(self,hdr,buf):
        return self.connector.get_psd_vector(hdr, buf)

    def scan(self,iface, psdq, debug):
        return self.connector.scan(iface,psdq, debug)

    def scan_dev_configure(self,iface,mode,count,period,fft_period,short_repeat):
        return self.ath9k_Scan_helper.scan_dev_configure(iface,mode,count,period,fft_period,short_repeat)

    def scan_dev_start(self,iface):
        return self.ath9k_Scan_helper.scan_dev_start(iface)

    def scan_dev_stop(self,iface):
        return self.ath9k_Scan_helper.scan_dev_stop(iface)
