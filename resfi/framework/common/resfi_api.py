...
class AbstractResFiApp(ResFiNorthBoundAPI, Thread):
    ...
    def get_psd_vector(self,hdr,buf):
        return self.agent.get_psd_vector(hdr,buf)

    def scan_dev_configure(self,iface,mode,count,period,fft_period,short_repeat):
        return self.agent.scan_dev_configure(iface,mode,count,period,fft_period,short_repeat)

    def scan_dev_start(self,iface):
        return self.agent.scan_dev_start(iface)

    def scan_dev_stop(self,iface):
        return self.agent.scan_dev_stop(iface)

    def scan(self,iface, psdq, debug):
        return self.agent.scan(iface, psdq, debug)
